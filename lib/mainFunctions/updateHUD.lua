...
local function checkForUpdates()
    ...
    if hasUpdates and updatedFiles > 0 then
        -- Verifica se o updateHUD.lua foi atualizado para recarregar a configuração
        ...
        -- Se o updateHUD.lua foi atualizado, recarrega a configuração e atualiza o ícone
        if updateHUDWasUpdated then
            ...
            if file then
                ...
                local newIconId = content:match('ICON_ID%s*=%s*(%d+)')
